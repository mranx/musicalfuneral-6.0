...
import { ChevronDown, Contact, Loader2 } from 'lucide-react';
...
interface AnglicanPreferenceFormProps {
  existingData?: any;
  orderId?: string;
  mode?: 'create' | 'edit';
}
...
export default function AnglicanPreferenceForm({
  ...
  const [vocalSections, setVocalSections] = useState({
    startOfRosary: [
      ...
      { id: '2', name: 'Vocal Name 2', checked: false, duration: '0:30 sec', audioFile: 'audio2.mp3' },
      { id: '3', name: 'Vocal Name 3', checked: false, duration: '0:30 sec', audioFile: 'audio3.mp3' },
      { id: '4', name: 'Vocal Name 4', checked: false, duration: '0:30 sec', audioFile: 'audio4.mp3' },
    ],
    Processional: [
  ...
  // Load saved data based on mode and existing data
  useEffect(() => {
    const loadSavedData = async () => {
      try {
        ...
        if (existingData) {
          // Use existing data passed from parent component (edit mode)
          if (existingData.vocalSelections) {
            Object.entries(existingData.vocalSelections).forEach(([section, selection]: [string, any]) => {
          ...
          if (existingData.congregationalHymn) {
            setSelectionState(prev => ({
              ...prev,
              congregationalHymn: existingData.congregationalHymn
            }));
          }
          
          // Restore additional options
          if (existingData.additionalOptions) {
            setAdditionalOptions(existingData.additionalOptions);
          }
        } else {
          // If no existing data, try to load from API
          try {
            // Use the 'current' endpoint which will use the cookie
            const response = await axios.get('/api/forms/current');
            
            if (response.data.success) {
              // Save the orderId for later use if not provided as prop
              if (response.data.orderId && !orderId) {
                setOrderId(response.data.orderId);
              }
              
              // Check if anglicanPreference data exists
              const savedData = response.data.formData?.anglicanPreference;
    ...
    loadSavedData();
  }, [existingData, orderId, vocalSections]);
  ...
  const handleNextClick = async () => {
    ...
    if (validateForm()) {
      try {
        ...
        // Save the form data
        await axios.put('/api/forms/update', requestData);
        
        setSuccess('Service preferences saved successfully!');
        ...
        setError('Failed to save service preferences. Please try again.');
      } finally {
        setIsSaving(false);
      }
    } else {
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-between pt-4">
              <Button 
                ...
                onClick={() => mode === 'edit' ? router.push('/dashboard') : router.back()}
              >
                {mode === 'edit' ? 'Cancel' : 'Previous'}
              </Button>
              <Button 
                type="button" 
              ...
              >
                {isSaving ? (
                  ...
                  mode === 'edit' ? 'Update Preferences' : 'Next'
                )}
              </Button>
            </div>
